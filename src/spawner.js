...
module.exports = function (spawn) {
  context.keys().forEach(function (filename) {
    ...
    let creep_type_key = creep_template.key;

    let creeps_of_type = _.filter(Game.creeps, (creep) => creep.memory.creep_type == creep_type_key);
    ...
    memory['creep_type'] = creep_type_key;
    memory['state'] = 'Init';
    ...
    if (creeps_of_type.length < needed) {
      if (spawn.canCreateCreep(creep_template.body) == OK) {
        ...
        if (isNaN(result)) {
          console.log(`Spawning ${result} from Spawn1 as ${creep_type_key}`);
        }
      }
    }
  })
